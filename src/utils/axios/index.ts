...
const transform: AxiosTransform = {
  ...
  transformRequestHook: (res: AxiosResponse<Result>, options: RequestOptions) => {
    if (res.config.url!.indexOf('ticketlogin') > -1) return res.data
    const { data } = res
    if (!data) {
    ...
    // const { errcode, description, description_cn } = data
    ...
    // message.error({ content: timeoutMsg })
  ...
  requestInterceptors: (config) => {
    ...
    if (jump && import.meta.env.PROD) {
      window.parent?.postMessage('requesting', window.PUBLIC.URL_CONSOLE)
    }
  ...
  responseInterceptorsCatch: (_axiosInstance: AxiosResponse, error: any) => {
    const { response, code, message: _message } = error || {}
    const msg: string = response?.data?.error?.message ?? ''
    const err: string = error?.toString?.() ?? ''
    let errMessage = ''

    try {
      if (code === 'ECONNABORTED' && _message.indexOf('timeout') !== -1) {
