...
// 直接将 icon 注册成组件
// 用法：my-icon.svg  <ISvgMyIcon />
// 打包成静态组件
export default function createIcon() {
  const localPath = resolve(process.cwd(), './src/assets/svgComponents/');

  return Icons({
    compiler: 'vue3', //plain SVG content for vue3
    autoInstall: true, // 自动安装引入的组件库中的 icon
    defaultClass: 'iconify', //svg标签默认样式
    customCollections: {
      svg: FileSystemIconLoader(localPath, (svg) => svg.replace(/^<svg /, '<svg fill="currentColor" ')),
      inline: {
